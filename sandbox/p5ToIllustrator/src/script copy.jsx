 ...
 * I am using this is by sending the code directly from Visual Studio Code (VSCode) to Illustrator. That is possible installing
...
for (var i = 0; i < nodesData.nodes.length; i++) {
  ...
  for (var j = 0; j < theme.nodes.length; j++) {
    ...
    if (nodeName) {
      ...
      // add node
      addNode(sublayer, nodesData.nodes[i].nodes[j], {
        label: labelCharStyle,
        description: descriptionCharStyle,
      });
    }
  }
}
...
function addNode(iLayer, node, charStyles) {
  ...
  var textShortDescription = node.nodeShortDescription;

  // Create curve
  var circle = iLayer.pathItems.ellipse(
    y + h / 2,
    x - w / 2,
    w,
    h,
  ...
  // Label
  var rectRef = iLayer.pathItems.rectangle(y - 15, x - 21.5, 43, 40);
  var labelText = iLayer.textFrames.areaText(rectRef);
  labelText.contents = label;
  labelText.selected = true;

  // Short Description
  rectRef = iLayer.pathItems.rectangle(y - 55, x - 21.5, 43, 65);
  var descriptionTextA = iLayer.textFrames.areaText(rectRef);
  descriptionTextA.contents = textShortDescription;
  descriptionTextA.selected = true;

  // Description
  rectRef = iLayer.pathItems.rectangle(y - 95, x - 21.5, 43, 350);
  ...
  if (charStyles.description) {
    charStyles.description.applyTo(descriptionTextA.textRange);
...
/**
 * Custom made function to remove leading and trailing white space. https://community.adobe.com/t5/indesign/trim-function-not-working-in-extendscript-is-there-a-way-to-replicate-the-functioning/m-p/7671885
 * @param {string} str 
 * @returns 
 */
function trim(str) {
